...
_TIDY_ACTIONS_FORCED = (
    "这条是用户手动指定的，**必须离开活跃记忆**，所以本次只有两种处置："
    "extract=这条信息已能被事实覆盖 → 用 facts 提炼出来，原条移出常驻；"
    "archive=不再需要常驻（过期、一次性、已被取代）→ 直接移出常驻。"
    "这两种处置都可以顺带修正这条的 category / importance"
    "（觉得它该换个类别、或其实更重要，就一并改掉）。"
    "判断标准：能按需召回的信息不该占每轮的席位。"
    "若这条里含着必须长期遵守的约束，也请用 extract 把它落成事实"
    "（把约束写完整，重要的可用 importance 标出来），不要把它留在活跃。"
)
