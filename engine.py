...
REBUILD_BAN = (
    "本次为手动强制重新提取：这条**必须从活跃记忆里移出**，"
    "所以既不允许输出 keep，也不允许输出 split —— 只能给 extract 或 archive。"
    "判断标准仍是上面那套原规则：信息还能被用到就用 extract（事实尽量写全，"
    "重要的可标 importance），确实不再需要就 archive。"
    "若这条里含着必须长期遵守的约束，也请用 extract 把它落成事实（别忘了写 reason），"
    "不要为了把它留在活跃而 split。"
)
