...
def test_no_bold_markers_in_whole_schema():
    """★ 设置面板按**纯文本**渲染 ⇒ schema 里**任何**字符串都不许有 ** ✓
    （原来只守 help/description ✗ 漏了 default —— 事实合并提示词里就藏了 3 处 ✓）"""
    import json as _json
    import pathlib as _p

    src = (_p.Path(__file__).resolve().parents[1] / "schema.json").read_text(encoding="utf-8")
    bad = []
    ...
    walk(_json.loads(src))
    assert not bad, "这些位置的文案含加强符号（面板无法渲染 ✗）：%s" % bad


def test_no_bold_markers_in_panel_copy():
    """面板自己的文案（HTML）也不许有 ** ✓（事实体检那段就踩过 ✓）"""
    import pathlib as _p

    root = _p.Path(__file__).resolve().parents[1] / "web"
    for name in ("index.html",):
        ...
        assert not bad, "%s 里仍有 ** ⇒ 面板无法渲染 ✗：%s" % (name, bad[:3])
